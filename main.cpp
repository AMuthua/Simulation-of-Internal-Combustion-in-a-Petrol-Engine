...
// title
    glColor3f(0.0,0.0,1.0);
    text(x-250,y+415,"Internal Combustion of a Petrol Engine",2);
 y+=150;
    smallboard(x+700,y);
    glColor3f(0.3,0.3,0.3);
...
float speedKmh = 0.0f;  // Engine speed in Km/h
const float speedStep = 5.0f;  // Speed increment per key press
const float maxSpeed = 300.0f; // Max engine speed
const float minSpeed = 0.0f; // Min engine speed


void renderSpeed() {
    char speedText[50];
    sprintf(speedText, "Speed: %.1f Km/h", speedKmh);

    glColor3f(1.0, 1.0, 1.0); // White text
    glRasterPos2f(-0.9f, 0.9f); // Position in the top-left
    for (char *c = speedText; *c != '\0'; c++) {
...
int main(int argc,char **argv)
{
	...
	glutCreateWindow("Combustion Engine Simulation");
